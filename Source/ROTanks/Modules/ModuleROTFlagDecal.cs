...
namespace ROTanks
{
    public class ModuleROTFlagDecal : PartModule, IPartGeometryUpdated
    {
        ...
        private void onFlagToggled(bool updateSymmetry)
        {
            ...
            if (updateSymmetry)
            {
                ...
                foreach (Part p in part.symmetryCounterparts) { ((ModuleROTFlagDecal)p.Modules[index]).onFlagToggled(false); }
            }
        }
