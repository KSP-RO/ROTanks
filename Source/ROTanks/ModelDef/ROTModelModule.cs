...
namespace ROTanks
{
    ...
    public class ROTModelModule<U> where U : PartModule
    {
        ...
        {
            ROTUtils.destroyChildrenImmediate(root);
        ...
        public void textureSetSelected(BaseField field, System.Object oldValue)
        {
            actionWithSymmetry(m =>
            {
                ...
                m.applyTextureSet(m.textureSetName, !ROTGameSettings.persistRecolor());
        ...
        public void updateSelections()
        {
            if (modelField != null)
            {
                ...
                string[] names = ROTUtils.getNames(availableOptions, s => s.definition.name);
                string[] displays = ROTUtils.getNames(availableOptions, s => s.definition.title);
        ...
        public void updateAttachNodeBody(String[] nodeNames, bool userInput)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                if (i < nodeCount)
                {
                    ...
                    {
                        ROTAttachNodeUtils.createAttachNode(part, nodeNames[i], pos, orient, size);
                    }
        ...
        private void constructSubModels(Transform parent)
        {
            ...
            {
                smd = smds[i];
                clonedModel = ROTUtils.cloneModel(smd.modelURL);
                if (clonedModel == null)
                {
